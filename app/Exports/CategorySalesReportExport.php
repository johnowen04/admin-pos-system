...
class CategorySalesReportExport implements FromCollection, WithHeadings, WithMapping, WithEvents, ShouldAutoSize
{
    protected $startDate;
    protected $endDate;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle('A9:F9')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('ffced8f2'); // green
                $sheet->getStyle('A9:F9')->getFont()->getColor()->setARGB('ff2a3e80'); // black font
                $sheet->getStyle('A9:F9')->getFont()->setBold(true);

                $sheet->setCellValue('F1', $this->createdBy);
                $sheet->setCellValue('F2', $this->selectedOutletName);

                $sheet->setCellValue('E4', "Total Penjualan");
                $sheet->setCellValue('F4', $this->totalSales);

                $sheet->setCellValue('E5', "Total Produk Terjual");
                $sheet->setCellValue('F5', $this->totalQuantity);

                $sheet->setCellValue('F7', "Date Generated " . now()->format('Y-m-d H:i:s'));

                // Apply number formatting (e.g., currency with thousands separator) on totals
                $sheet->getStyle('F4')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            },
